...
def load_question_answer(opts):
    ...
    print("Training Data",len(data['training_data']))
    return data
def loadtest_question_answer(opts):
    load_data = np.load('./cocoqa/test.npy',encoding='bytes')
    testing_data = []
    for data_id in range(len(load_data[0])-42):
        testing_data.append({
        'image_id' : load_data[0][data_id][0][0], 
        'question' : np.zeros(55),
        'answer' : load_data[1][data_id][0]
        ...
        for question_id in range(55):
            testing_data[-1]['question'][question_id] = load_data[0][data_id][question_id+1][0]
    ques_vocab = get_ques_vocab('./cocoqa/vocab-dict.npy')
    ans_vocab = get_answer_vocab('./cocoqa/ansdict.pkl')
    data = {
    ...
    print("Testing Data", len(data['testing_data']))
    return data
