...
class Model:
	...
	def build_model():
		...
		lstm_q_vec = tf.placeholder(tf.float32, [self.batch_size, self.options.lstm_steps-1, 512])
		lstm_answer = tf.placeholder(tf.float32, [self.options.batch_size, self.options.ans_vocab_size], name = "answer")

		# LSTM preprocessing
		image_emb = tf.matmul(lstm_image_feat, Wimg_emb) + Bimg_emb
		...
		X_in = tf.matmul(X, Wemb__hidden) + Bemb_hidden
		...
		logits = tf.matmul(final_state[1][1], Whidden_ans) + Bhidden_ans
